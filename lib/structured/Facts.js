...
class Facts extends DocumentWorker {
    ...
    processElement(element) {
        ...
        if (this.isTopLevelSection(tagName, element)) {
            if (this.currentSection) {
                if (this.currentSection.paragraphs.length > 0
                    && this.currentSection.paragraphs[0].facts.length > 0) {
    ...
    processParagraph(paragraph) {
        if (this.currentParagraph) {
            if (this.currentFact && this.currentFact.text !== '') {
                this.currentFact.text = this.currentFact.text.trim();
                this.currentParagraph.facts.push(this.currentFact);
                this.currentFact = this.newFact();
            }
